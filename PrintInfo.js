...
export async function searchAirportInfo(city){
  const airportInfo = await getAirport(city);
  ...
  for (let i = 0; i<airportInfo.items.length; i++){
    ...
    button.onclick = function() {chooseAirport(airportInfo.items[i].icao, airportInfo.items[i].name)};
...
function chooseAirport(ICAO, airportName) {
  const div = document.getElementById("Buttons");
  div.innerHTML = "";
  printArrDep(ICAO, airportName);
}

//This function prints out the next 5 departures and arrivals of the selected airport. -Tuomas
export async function printArrDep(ICAO, airportName){
  const contains = await getArrDep(ICAO);
  ...
  for (let i = 0; i < 5; i++) {
    if (contains.departures[i] === undefined) {
      ...
      const departureTime = Date.getTime(
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const arrivalTime = Date.getTime(contains.arrivals[i].movement.scheduledTimeLocal);
...
async function printFlight() {
  ...
  if (contents.length > 0) {
    ...
    let arrivalTime = Date.getTime(contents[0].arrival.scheduledTimeLocal);
    resetText(2);
...
async function printDelays(ICAO){
  const contents = await getDelays(ICAO);
